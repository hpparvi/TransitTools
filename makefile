...
all: transitmodel/gimenez.f90 Fortran/utilities.f90 fitting/fitnessfunction.f90
	...
	f2py -c --opt=${OPTS} ${CMP} -m fitnessfunction_f fitting/fitnessfunction.f90 ${FLAGS}
	f2py -c --opt=${OPTS} ${CMP} -m utilities_f Fortran/utilities.f90 ${FLAGS}
...
openmp: transitmodel/gimenez.f90 Fortran/utilities.f90 fitting/fitnessfunction.f90
	...
	f2py -c --opt=${OPTS_OMP} ${CMP} -lgomp -m fitnessfunction_f fitting/fitnessfunction.f90 ${FLAGS}
	f2py -c --opt=${OPTS_OMP} ${CMP} -lgomp -m utilities_f Fortran/utilities.f90 ${FLAGS}
